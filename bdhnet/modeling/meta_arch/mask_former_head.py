...
class MaskFormerHead(nn.Module):
    ...
    def forward(self, features, mask=None, scale=1.0):
        return self.layers(features, mask, scale)

    @get_local("mask_features")
    def layers(self, features, mask=None, scale=1.0):
        mask_features, transformer_encoder_features, multi_scale_features = self.pixel_decoder.forward_features(features)
        if self.transformer_in_feature == "multi_scale_pixel_decoder":
            predictions = self.predictor(multi_scale_features, mask_features, mask, scale)
        else:
